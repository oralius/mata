				...
				if #msg.content_.members_ > 0 then
					if not hsh then
						if not lang then
							...
							tdcli.sendMessage(msg.to.id, 0, 1, username..'\n_شما تعداد '..(#msg.content_.members_ + 1)..' عضو را اضافه کردید!_\n_اما فقط یک عضو برای شما ذخیره شد!_\n_لطفا مخاطبین را تک به تک اضافه کنید تا محدودیت برای شما حذف شود_', 1, 'md')
						end
					end
				end
				...
				if not chk then
					...
					if tonumber(permit) < tonumber(setadd) then
						...
						if not hsh then
							if not lang then
								...
								tdcli.sendMessage(msg.to.id, 0, 1, username..'\n_شما تعداد_ `'..permit..'` _عضو را به این گروه اضافه کردید._\n_باید_ `'..less..'` _عضو دیگر برای رفع محدودیت چت اضافه کنید._', 1, 'md')
							end
						end
					...
					if not hsh then
						if not lang then
							...
							tdcli.sendMessage(msg.to.id, 0, 1, username..'\n_شما قبلا این مخاطب را به گروه اضافه* کرده اید!*_', 1, 'md')
						end
					end
				end
			end
			...
			if tonumber(permit) < tonumber(setadd) then
				...
				if not chkpm then
					...
					if not lang then
						...
						tdcli.sendMessage(msg.to.id, 0, 1, (check_markdown(msg.from.username) or msg.from.print_name)..'\n_شما باید_* '..setadd..' _عضو دیگری را به این گروه دعوت کنید تا بتوانید پیام ارسال کنید*_', 1, 'md')
					end
				end
			end
		end
	...
	if is_mod(msg) then
			...
			if parts[2]:lower() == 'add' then
				...
				if add == 'on' then 
					...
					if not lang then
						...
						return '*محدودیت افزودن عضو فعلا غیر فعال شد*'
					end
				elseif add == 'off' then
					if not lang then
						return '*Limit Add Member Is Already* `Unlocked`'
					else
						return '* محدودیت افزودن عضو قبلا غیر فعال شده بود*'
					end
				end
			end
		end
			...
			if parts[2]:lower() == 'add' then
				...
				if add == 'off' then 
					...
					if not lang then
						...
						return '*محدودیت افزودن عضو در این گروه فعال شد*'
					end
				elseif add == 'on' then
					if not lang then
						return '*Limit Add Member Is Already* `Locked`'
					else
						return '*محدودیت افزودن عضو از قبل فعال بوده است*'
					end
				end
			end
		end
		if parts[1]:lower() == 'setadd' and parts[2] then
			if tonumber(parts[2]) >= 1 and tonumber(parts[2]) <= 10 then
				...
				if not lang then
					...
					return '*تنظیم محدودیت اضافه کردن عضو به:* `'.. parts[2]..'`'
				end
			else
				if not lang then
		...
		if parts[1]:lower() == 'getadd' then
			...
			if not lang then
				...
				return '*محدودیت اضافه کردن عضو:* `'..getadd..'`'
			end
		end
